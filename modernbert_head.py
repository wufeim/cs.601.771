...
def build_dataset(args, tokenizer, collator):
    ...
    def preprocess(batch):
        ...
        result["label"] = 1 if batch["answer"] else 0
        return result
...
def build_model(args, num_labels=2):
    id2label = {0: "false", 1: "true"}
    label2id = {v: k for k, v in id2label.items()}
    config = AutoConfig.from_pretrained(args.model_name, num_labels=num_labels, id2label=id2label, label2id=label2id)
